...
try:
    ...
    AWARN(
        'You need to install youtube-dl to use parts of VideoSource class that pull from YouTube. Try running:\npip install youtube-dl')
...
class VideoSource(AFileManager):
    ...
    def getDirForVersion(self, version_label=None, version_group=None):
        vdir = self.getDir('versions') + os.sep + self._versionGroupString(
            version_group) + os.sep + self._getVersionLabelDirName(version_label) + os.sep
        return vdir
    ...
    def getVersion(self, max_height=None, get_if_missing=True, load_features=True, **kwargs):
        ...
        if (vpath and os.path.isfile(vpath)):
            ...
            if (num_frames_total):
                v = self.RegisteredVideo(path=vpath, version_label=max_height, num_frames_total=num_frames_total,
                                         load_features=True);
            else:
            ...
            if (get_if_missing):
                ...
                if (vpath):
                    num_frames_total = self.getVersionInfo(version_label=max_height, info_label='num_frames_total');
                    if (num_frames_total):
                        v = self.RegisteredVideo(path=vpath, version_label=max_height,
                                                 num_frames_total=num_frames_total, load_features=True);
                    else:
                        v = self.RegisteredVideo(path=vpath, version_label=max_height, load_features=True);
                        self.setVersionInfo(version_label=max_height, num_frames_total=v.num_frames_total);
                        self.save();
    ...
    def saveFeatureForVideo(self, video, feature_name, output_dir=None, overwrite=True):
        if (feature_name == 'each'):
            return self.saveFeaturesForVideo(video=video, features_to_save=video.getFeaturesList(),
                                             output_dir=output_dir, overwrite=overwrite);

        version_label = video.getInfo(label='version_label');
        source_type = video.getFeatureSourceType(feature_name);
        opath = self.getFeaturePath(feature_name=feature_name, version_label=version_label, source_type=source_type,
    ...
    def RegisteredVideo(self, path=None, version_label=None, version_group=None, num_frames_total=None,
                        load_features=True):
        v = self.VideoClass(path=path,
                            name=self.getName() + '_' + self._versionLabelString(version_label=version_label),
    ...
    def RegisterVideo(self, video, version_label=None, version_group=None, load_features=True):
        def videosave(vidob, features_to_save='all', output_dir=None, overwrite=True):
            self.saveFeaturesForVideo(video=vidob, features_to_save=features_to_save, output_dir=output_dir,
                                      overwrite=overwrite);
            # self.save
    ...
    def pullYoutubeVideo(self, max_height=None, write_subtitles=False, save_youtube_info=False,
        ...
        :return:
        """
        assert (not self.getInfo('source_is_file')), "tried to call pullYoutubeVideo on file source {}.".format(
            self.getName());
        ...
        output_dir = self.getDirForVersion(version_label=max_height,
                                           version_group=None)
        ...
        if (not force_redownload) and self.video_file_name is not None:
            ...
            if os.path.isfile(path_guess):
                AWARN(
                    "found existing file {}\nSkipping download; set force_redownload=True to overwrite old version.".format(
                        path_guess))

...
def MPYWriteVideoFile(mpyclip, filename, **kwargs):
    temp_audio_filename = get_temp_file_path(final_file_path='TEMP_' + filename + '.m4a',
                                             temp_dir_path=Video.VIDEO_TEMP_DIR)
...
class Video(TimeSignal):
    ...
    def calcNumFramesTotal(self):
        ...
        if float('inf') == self.reader.get_length():
            i = 0
            while True:
                try:
    ...
    def writeResolutionCopyFFMPEG(self, path, max_height=None):
        ...
        mpc = self.getMPYClip();
        clip_resized = mpc.resize(
            height=max_height);  # make the height 360px ( According to moviePy documenation The width is then computed so that the width/height ratio is conserved.)
    ...
    def writeResolutionCopy(self, path, max_height=None, reshape=None, input_sampling_rate_factor=None,
    ...
    def writeWarped(self, output_path, warp, output_sampling_rate=None, output_audio=None, bitrate=None, vbmark=True,
        ...
        cropped_output_audio = output_audio.NewAudioClip(start=target_start, end=target_end)
    ...
    def getWarped(self, target,
        ...
        if not os.path.isfile(output_path) or force_recompute:
            rvid = self.writeWarped(output_path=output_path,
                                    warp=warp,
    ...
    def getWithSoundsOnEvents(self, events, output_path=None, name_tag=None, mute_original=True,
        ...
        if (output_path is None):
            if (self.source is not None):
                output_dir = self.source.getDirForVersion(version_label=self.getInfo('version_label'),
                                                          version_group='sound_on_events');
            else:
    ...
    @staticmethod
    def CreateByStackingVideos(video_objects=None, video_paths=None, output_path=None, audio=None, concatdim=0,
        ...
        rvid = Video.CreateFromVideoAndAudio(video_path=tempfilepath, audio_object=audio, output_path=output_path,
                                             **kwargs);
        os.remove(tempfilepath);
        return rvid;
    ...
    def CreateFromVideoAndAudioPaths(video_path, audio_path, output_path, return_vid=True, **kwargs):
        return Video.CreateFromVideoAndAudio(video_path=video_path, audio_path=audio_path, output_path=output_path,
    ...
    def CreateFromVideoAndAudioObjects(video, audio, output_path, clip_to_video_length=True, return_vid=True, **kwargs):
        return Video.CreateFromVideoAndAudio(video_object=video, audio_object=audio, output_path=output_path,
                                             clip_to_video_length=clip_to_video_length, return_vid=return_vid,
                                             **kwargs);
...
if (BeatGUI.VIEWER_INSTALLED):
    ...
    def runBeatGUIOnAudio(self):
        audio = self.getAudio();
        self.gui.run(local_saliency=audio.getLocalRhythmicSaliency(), frame_rate=audio._getFrameRate(),
                     eventlist=audio.getEventList());


    def runGUI(self, local_saliency=None, frame_rate=None, eventlist='default', frame_offset=None):
        self.gui.run(local_saliency=local_saliency, frame_rate=frame_rate, eventlist=eventlist,

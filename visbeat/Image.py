...
class Image(
    ...
    def getScaled(self, shape=None, shape_xy=None):
        ...
        assert ((shapeis or shapexyis) and not (
                shapeis and shapexyis)), "Must provide only one of shape or shape_xy for Image.getScaled"
        if (shapeis):
    ...
    def _splatAtPixCoord(self, im, location=[0, 0], **kwargs):
        ...
        if (im.n_channels == 4):
            ...
            blenda = (im._pixels_float[:, :, :3]) * alphamap + self._pixels_float[region0[0]:region0[1],
                                                               region1[0]:region1[1], :] * (1.0 - alphamap);
            if (is_int):

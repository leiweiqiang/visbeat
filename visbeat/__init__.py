...
def PullVideo(name=None, source_location=None, max_height=240, **kwargs):
    if isinstance(name, VisBeatExampleVideo):
        ...
        assert (source_location is not None), "Must provide an argument to pull video"
    ...
    vs = VideoSource.NewVideoSource(destination=GetVideoSourcesDir(),
                                    name=vname,
...
def ClipVideo(video, time_range, max_height=240):
    ...
    vcdir = video.source.getDirForVersion(version_label='{}_{}'.format(str(time_range[0]), str(time_range[1])),
                                          version_group='Clips');
...
def AutoDancefer(source,
    ...
    result = Dancefer(source_video=sourcev,
                      target=targetv,
